    ...
    try:
        ...
        for row in logs:
            ...
            if is_demo_phone(phone):
                continue
            row["phone_number"] = phone or row.get("phone_number")
            cleaned.append(row)
        logs = cleaned
        return logs
...
async def api_get_analytics():
    """Analytics endpoint for SPA charts and KPI cards."""
    config = read_config()
    os.environ["SUPABASE_URL"] = config.get("supabase_url", "")
    os.environ["SUPABASE_KEY"] = config.get("supabase_key", "")
    import db
    ...
    try:
        ...
        outcomes = {"booked": 0, "cancelled": 0, "completed": 0, "unknown": 0}
        ...
        avg_duration = round(total_duration / connected_calls) if connected_calls else 0
        ...
        daily_series = sorted(
            [{"date": d, "calls": c} for d, c in by_day.items()],
            key=lambda x: x["date"],
        )[-14:]
        hourly_series = [
            {"hour": h, "calls": by_hour.get(h, 0)}
            for h in range(24)
        ]
        ...
        return {
            "kpis": {"total_calls": 0, "booked_calls": 0, "booking_rate": 0.0, "connect_rate": 0.0, "avg_duration": 0},
            "outcomes": {"booked": 0, "cancelled": 0, "completed": 0, "unknown": 0},
            "daily_series": [],
            "hourly_series": [],
        }
...
async def api_get_patients():
    """Patients endpoint derived from existing contacts/logs for phase 1."""
    config = read_config()
    os.environ["SUPABASE_URL"] = config.get("supabase_url", "")
    os.environ["SUPABASE_KEY"] = config.get("supabase_key", "")
    try:
        contact_rows = await api_get_contacts()
        if not contact_rows:
            return []

        # Add lightweight profile fields for UI presentation.
        patients = []
        for row in contact_rows:
            total_calls = int(row.get("total_calls") or 0)
            is_booked = bool(row.get("is_booked"))
            engagement = "high" if total_calls >= 5 else ("medium" if total_calls >= 2 else "low")
            status = "active" if is_booked or total_calls > 0 else "new"
            patients.append(
                {
                    "patient_id": (row.get("phone_number") or "unknown").replace("+", ""),
                    "name": row.get("caller_name") or "Unknown Patient",
...
async def api_call_single(request: Request):
    ...
    if not is_valid_phone_number(phone):
        return {
            "status": "error",
            "message": "Invalid phone number. For India use +91XXXXXXXXXX, 91XXXXXXXXXX, or XXXXXXXXXX (10 digits).",
        }
    config = read_config()
    try:
            ...
            results.append(
                {
                    "phone": phone,
                    "status": "error",
                    "message": "Invalid phone number. India format: +91XXXXXXXXXX / 91XXXXXXXXXX / XXXXXXXXXX.",
                }
            )
            continue
        try:
...
try:
    ...
    def _get_or_create_metric(metric_cls, name, documentation, **kwargs):
        existing = REGISTRY._names_to_collectors.get(name)
        if existing is not None:
            return existing
        return metric_cls(name, documentation, **kwargs)

    _voice_calls_total = _get_or_create_metric(
        Counter,
        "voice_calls_total",
        "Total calls handled by the agent",
    )
    _voice_calls_booked = _get_or_create_metric(
        Counter,
        "voice_calls_booked_total",
        "Calls that resulted in a booking",
    )
    _voice_call_duration = _get_or_create_metric(
        Histogram,
        "voice_call_duration_seconds",
        "Call duration in seconds",
        buckets=[10, 30, 60, 120, 300, 600, 1200],
    )
    _voice_calls_active = _get_or_create_metric(
        Gauge,
        "voice_calls_active",
